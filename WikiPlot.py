...
def plot_prediction(encode, target, prediction, tail_len=50):
    ...
    plt.subplot(1, 2, 1)
    plot_series(encode_series_tail, title=f"Series with tail of {tail_len}", days=range(1, x_encode + 1), color='blue', new_plot=False, plot_show=False, label='history')
    ...
    plt.subplot(1, 2, 2)
    plot_series(encode, title=f"Full series", days=range(1, x_encode + 1), color='blue', new_plot=False, plot_show=False, label='history')
    plot_series(target, days=range(x_encode, x_encode + pred_steps), color='orange', new_plot=False, plot_show=False, label='target')
    plot_series(prediction, days=range(x_encode, x_encode + pred_steps), color='teal', linestyle='--', new_plot=False, plot_show=False, label='prediction')
    plt.show()
